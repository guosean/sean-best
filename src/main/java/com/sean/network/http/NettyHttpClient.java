...
//            _sslCtx = SslContextBuilder.forClient().trustManager(InsecureTrustManagerFactory.INSTANCE).build();
            ...
            b.handler(new NettyClientInitializer(null,null,null));
    ...
    private ResponseWrapper sendHttpRequest(HttpMethod method, String url, String body) throws APIConnectionException,
            APIRequestException, URISyntaxException, InterruptedException {
        ...
        try {
            SettableFuture<ResponseWrapper> future = SettableFuture.create();
            ChannelFuture connect = b.connect(host, port);
            Channel _channel = connect.sync().channel();

            HttpResponseHandler responseHandler2 = new HttpResponseHandler(future);
            _channel.pipeline().addLast(responseHandler2);
            FullHttpRequest request;
            if (null != body) {
...
//                throw new APIRequestException(wrapper);
    ...
    public interface BaseCallback {
        void onSucceed(ResponseWrapper wrapper);
    }
}
